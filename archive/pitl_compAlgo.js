...
var scoreData_fileNamePrefix = "pitl_" + t_now.getFullYear() + "_" + month + "_" + t_now.getUTCDate() + "_" + t_now.getHours() + "-" + t_now.getMinutes() + "-" + t_now.getSeconds() ;
...
let cresDurs_str = JSON.stringify(cresDurs);
downloadStrToHD(cresDurs_str, 'cresDurs.txt', 'text/plain');

let sec3HocketPlayers_str = JSON.stringify(sec3HocketPlayers);
downloadStrToHD(sec3HocketPlayers_str, 'sec3HocketPlayers.txt', 'text/plain');

let sec3Cres_str = JSON.stringify(sec3Cres);
downloadStrToHD(sec3Cres_str, 'sec3Cres.txt', 'text/plain');

let sec3Accel_str = JSON.stringify(sec3Accel);
downloadStrToHD(sec3Accel_str, 'sec3Accel.txt', 'text/plain');

// ***** SEE PITCH CHANGES ABOVE pitchChanges RUN IN ASYNC METHOD

let varsArr = [sec2start, endSec2Time, sec3StartTime, sec3EndTime];
